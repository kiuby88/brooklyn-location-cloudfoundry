...
public abstract class CloudFoundryWebAppImpl extends AbstractEntity
        implements CloudFoundryWebApp, DriverDependentEntity {

    private static final Logger log = LoggerFactory.getLogger(CloudFoundryWebAppImpl.class);

    private transient PaasWebAppDriver driver;
    ...
    protected void initDriver(CloudFoundryPaasLocation location) {
        PaasWebAppDriver newDriver = doInitDriver(location);
        if (newDriver == null) {
     ...
     */
    protected PaasWebAppDriver doInitDriver(CloudFoundryPaasLocation location) {
        if (driver != null) {
    ...
    protected PaasWebAppDriver newDriver(CloudFoundryPaasLocation loc) {
        EntityDriverManager entityDriverManager = getManagementContext().getEntityDriverManager();
        return (PaasWebAppDriver) entityDriverManager.build(this, loc);
    }
                    ...
                    try {
                        if (getManagementSupport().isNoLongerManaged()) {
                            log.debug("Entity {} no longer managed; ignoring scheduled connect " +
                                    "sensors on rebind", CloudFoundryWebAppImpl.this);
                            return;
                        }
                        connectSensors();
                    } catch (Throwable e) {
                        log.warn("Problem connecting sensors on rebind of " +
                                CloudFoundryWebAppImpl.this, e);
