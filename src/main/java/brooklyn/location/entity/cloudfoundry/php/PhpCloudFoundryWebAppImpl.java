 ...
 * under the License.
 */
package brooklyn.location.entity.cloudfoundry.php;
...
public class PhpCloudFoundryWebAppImpl extends AbstractEntity
        implements PhpCloudFoundryWebApp, DriverDependentEntity {


    private static final Logger log = LoggerFactory.getLogger(PhpCloudFoundryWebAppImpl.class);

    private transient PhpWebAppDriver driver;
    ...
    public PhpCloudFoundryWebAppImpl() {
        super(MutableMap.of(), null);
    }

    public PhpCloudFoundryWebAppImpl(Entity parent) {
        this(MutableMap.of(), parent);
    }

    public PhpCloudFoundryWebAppImpl(Map properties) {
        this(properties, null);
    }

    public PhpCloudFoundryWebAppImpl(Map properties, Entity parent) {
    ...
    /**
     * If custom behaviour is required by sub-classes, consider overriding {@link #doStart(java.util.Collection)})}.
     */
    @Override
    ...
    protected void initDriver(CloudFoundryPaasLocation location) {
        PhpWebAppDriver newDriver = doInitDriver(location);
        if (newDriver == null) {
     ...
     */
    protected PhpWebAppDriver doInitDriver(CloudFoundryPaasLocation location) {
        if (driver != null) {
    ...
    protected PhpWebAppDriver newDriver(CloudFoundryPaasLocation loc) {
        EntityDriverManager entityDriverManager = getManagementContext().getEntityDriverManager();
        return (PhpWebAppDriver) entityDriverManager.build(this, loc);
     ...
     * add do additional work via tasks, executed using {@link brooklyn.util.task.DynamicTasks#queue(String, java.util.concurrent.Callable)}.
     */
                    ...
                    try {
                        if (getManagementSupport().isNoLongerManaged()) {
                            log.debug("Entity {} no longer managed; ignoring scheduled connect " +
                                    "sensors on rebind", PhpCloudFoundryWebAppImpl.this);
                            return;
                        }
                        connectSensors();
                    } catch (Throwable e) {
                        log.warn("Problem connecting sensors on rebind of " +
                                PhpCloudFoundryWebAppImpl.this, e);
