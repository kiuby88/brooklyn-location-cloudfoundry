...
public class UserProvidedServiceCloudFoundryDriver extends PaasServiceCloudFoundryDriver  implements UserProvidedServiceDriver {
    ...
    @Override
    public void launch() {
        createUserProvidedService(getEntity().getServiceName(), getEntity().getCredentials());
    }
    ...
    public void createUserProvidedService(String serviceName, Map<String, Object> credentials){
        if (!isServiceCreated(serviceName)) {
            serviceInstance = new CloudService();
            serviceInstance.setName(serviceName);
            getClient().createUserProvidedService(
                    serviceInstance,
                    getEntity().getConfig(UserProvidedService.CREDENTIALS));
        } else {
